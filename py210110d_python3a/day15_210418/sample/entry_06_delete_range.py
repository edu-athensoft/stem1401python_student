...
insertbtn.grid(row=4, column=3, sticky="we")

clearbtn = Button(root, text="Clear All", command=clearInfoAll)
clearbtn.grid(row=5, column=2, sticky="we")

clearbtn = Button(root, text="Clear Specific Char", command=lambda: clearInfoChar(3))
clearbtn.grid(row=6, column=2, sticky="we")

quitbtn = Button(root, text="Quit", command=root.destroy)
quitbtn.grid(row=7, column=3, sticky="we")
